...
document.body.style.zoom = "90%";
        ...
        function clearCache() {
            ...
            document.getElementById('isPspCheckbox').checked = isPsp;
        }
            ...
            async function checkOrderStatus(orderId) {
                try {
                    ...
                    if (!authKey || !authKey.value) {
                        console.error('Auth key not found or empty');
                        return null;
                    }
                    if (!clientId || !clientId.value) {
                        console.error('Client ID not found or empty');
                        return null;
                    }

                    const headers = {
                        'Accept': 'application/json',
                        'X-Client-Secret': authKey.value,
                        'X-Client-ID': clientId.value
                    };

                    if (isPspCheckbox && isPspCheckbox.checked) {
                        const merchantId = document.getElementById('merchantId');
                        if (merchantId && merchantId.value) {
                            headers['X-Merchant-ID'] = merchantId.value;
                        }
                    }
                ...
                try {
                    ...
                    const response = await fetch(apiUrl, {
                        ...
                        headers: {
                            ...
                            ...(isPspCheckbox.checked && document.getElementById('merchantId').value ? {'X-Merchant-ID': document.getElementById('merchantId').value} : {})
                        },
